...
sanitycheck.generate <- function(n=100, prop=0.01, stream.config=NULL, verbose=FALSE) {
  ...
  if(!is.null(stream.config)) {
    ...
    if(stream.config$nsteps > 1) {
      ...
      if(length(stream.config$nsteps) != length(n) & length(n) != 1) {
        stop("Length of 'n' is not coherent with stream.config. Should be either 1 for a regular stream or a vector and same size as nsteps")
      }
      if(length(stream.config$nsteps) != length(n) & length(n) != 1) {
        stop("Length of 'n' is not coherent with stream.config. Should be either 1 for a regular stream or a vector and same size as nsteps")
      }
...
sanitycheck.config <- function(dim=20, mindim=2, maxdim=4, subspaces=NULL, nsteps=10, cycle=0, 
  ...
  if(nsteps <= 0) {
    stop("'nsteps' should be > 0. Note: 1 means a static stream")
  }
  if(nsteps==0 & cycle != 0) {
    stop("Defining a cycle does not make sense in a non-dynamic stream")
  }
  if(cycle < 0 | cycle > nsteps/2) {
    stop("The number of elements to create a cycle 'cycle' should be between 0 (no cycle) and half of the number of steps 'nsteps'")
  }
  if(dim < 10) {
  ...
  if(nsteps < 0) {
    stop("The number of steps 'nsteps' should be strictly positive")
  }
}